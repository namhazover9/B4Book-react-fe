...
const BookShowcase = () => {
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            setCurrentIndex((prev) => (prev + 1) % books.length); // Quay về ảnh đầu tiên khi đạt ảnh cuối cùng
        }, 4000);
        return () => clearInterval(timer);
    }, []);
    ...
    const visibleBooks = [
        ...
        ...books.slice(0, Math.max(0, (currentIndex + 4) - books.length)), // Hiển thị các ảnh đầu nếu currentIndex + 4 vượt quá độ dài books
    ];
